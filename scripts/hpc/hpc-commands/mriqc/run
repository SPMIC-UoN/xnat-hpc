...
JOB_ID=`sbatch --parsable $CODEDIR/mriqc/mriqc.slurm $BIDS_DATASETDIR $WORKDIR $MRIQC_SUBDIR`
echo "MRIQC job ID: $JOB_ID"

# Run dependent job to do upload
UPLOAD_JOB_ID=`sbatch --parsable --dependency=afterok:$JOB_ID $CODEDIR/mriqc/upload.slurm $HOST $PROJ $SUB $EXP MRIQC $WORKDIR $MRIQC_SUBDIR ${CREDENTIALS[0]} ${CREDENTIALS[1]}`
